...
export default function Home({ blogs, keyWritings, keyInitiatives, upcomingResidencies, podcasts, learnTopics }) {
    console.log("posts", blogs)
    console.log("keyWritings", keyWritings)
    console.log("keyInitiatives", keyInitiatives)
    console.log("upcomingResidencies", upcomingResidencies)
    console.log("podcasts", podcasts)
    console.log("learnTopics", learnTopics)
                            ...
                            <SplideTrack>
                                {podcasts?.map((podcast) => (
                                    <SplideSlide key={podcast._id} className="p-1">
                                        <Link href={`/${podcast.urlPath}`} className="flex flex-col overflow-hidden rounded-lg shadow-lg border">
...
export async function getStaticProps() {
    ...
    const blogs = await Promise.all(blogsMetadataPromises);
    ...
    for (const path of keyWritingsPaths) {
        const dbFile = await mddb.getFileByUrl(path)
        ...
        keyWritings.push(frontMatterWithComputedFields);
    }

    let keyInitiatives = []

    for (const path of keyInitiativesPaths) {
        const dbFile = await mddb.getFileByUrl(path)
        const filePath = dbFile.file_path;
        const frontMatter = dbFile.metadata ?? {};
        ...
        keyInitiatives.push(frontMatterWithComputedFields);
    }

    keyInitiatives = keyInitiatives.sort((a, b) => (new Date(b.created)).getTime() - (new Date(a.created)).getTime())

    let upcomingResidencies = []

    const residenciesFiles = await mddb.getFiles({ folder: "residencies" });
    ...
    upcomingResidencies = await Promise.all(residenciesMetadataPromises);
    upcomingResidencies = upcomingResidencies
        .filter(residency => new Date(residency.start).getTime() > (new Date().getTime()))

    let podcasts = []

    const podcastsFiles = await mddb.getFiles({ folder: "podcast" });

    const podcastsMetadataPromises = podcastsFiles.map(async (b) => {
        const source = fs.readFileSync(b.file_path, { encoding: "utf-8" });
    ...
    podcasts = await Promise.all(podcastsMetadataPromises);
    podcasts = podcasts.sort((a, b) => (new Date(b.date)).getTime() - (new Date(a.date)).getTime())
    ...
    return {
        props: {
            blogs, keyWritings, keyInitiatives, upcomingResidencies, podcasts, learnTopics
        }
    }
}
